...
public class AccountController {
    ...
    public String viewSaveAccount(@ModelAttribute(value="account")Account account,@RequestParam String checkcode, HttpSession session,Model model){
        boolean lang2=true;
        model.addAttribute("lang2",lang2);
        if(session.getAttribute("imageCode").toString().equalsIgnoreCase(checkcode)) {
           accountService.insertAccount(account);
           return "catalog/main";
       }else return "account/NewAccountForm";
    }
   ...
   @GetMapping("/sign/language")
    ...
    @GetMapping("/account/myLog")
    public String myOrder(HttpServletRequest request, Model model) {
        HttpSession session = request.getSession();
        Account account = (Account)session.getAttribute("account");
        List<Log> logList = logService.getLogsByUserId(account.getUsername());
        model.addAttribute("logList",logList);
        return  "account/myLog";
    }
}
